...
if(this.state.higherlevelgroups){higherlevelgroupsmapped=this.state.higherlevelgroups.map(item => {
      ...
      return(
        <><div key={item._id}>
          <Link exact to={"groups/" + item._id+"/"+"groups"}>
                      <div>
                       <h3>{item.location}</h3>
                      </div></Link>
                      ...
                      <MapContainer center={[item.centroid[0],item.centroid[1]]} zoom={13} scrollWheelZoom={false}>
                        ...
                        />
                          <CircleMarker center={[item.centroid[0],item.centroid[1]]} pathOptions={redOptions} radius={item.radius/1000}>
                          </CircleMarker>
                      </MapContainer></>
      )
    })}
    ...
    if(this.state.localgroups){localgroupsmapped=this.state.localgroups.map(item => {
      ...
      return(
        <><div key={this.state.localgroup._id}>
          <Link exact to={"groups/" + this.state.localgroup._id+"/"+"localgroup"}>
                      <div>
...
if(this.state.groups){groupsmapped=this.state.groups.map(item => {
      ...
      return(
        <>
        <div key={item._id}>
<Link exact to={"groups/" + item._id+"/"+"groups"}><h3>{item.name}</h3>
            <div>
             <h3>{item.title}</h3>
    ...
    return (
      <React.Fragment>
        ...
        <section>
          ...
          <button onClick={this.viewYourLocalGroup}>View Your Local Group?</button>
  ...
  <Link exact to={"groups/" + this.state.localgroup._id+"/"+"localgroup"}>
              ...
              <MapContainer center={[this.state.localgroup.centroid[0],this.state.localgroup.centroid[1]]} zoom={13} scrollWheelZoom={false}>
                <TileLayer
                  ...
                  url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                />
                  <CircleMarker center={[this.state.localgroup.centroid[0],this.state.localgroup.centroid[1]]} pathOptions={redOptions} radius={this.state.localgroup.radius/1000}>
                  </CircleMarker>
              </MapContainer></>}
              <hr/>
...
{this.state.viewalllocalgroups&&localgroupsmapped}
          ...
          <h3>Groups</h3>
          <br/>
          <button onClick={this.viewAllGroups}>View All Groups?</button>
